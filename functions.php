...
function dmeng_script_uri( $name = '' , $speed = 'auto' ){
	...
	$dmeng_script_file_name = 'huoxing-1.0.1';
	...
	switch( $name ){
		...
		case 'grey_png' :
			$path = $speed ? '/grey.png' : '/images/grey.gif';
			break;
...
function dmeng_setup() {
	...
	register_nav_menus( array(
		...
		'left_menu' => __( '首页左侧导航菜单', 'dmeng' ),
		'footer_menu' => __( '页脚链接菜单', 'dmeng' ),
		'page_menu' => __( 'Page页菜单', 'dmeng' ),
		'mobile_menu' => __( '手机端滑动菜单', 'dmeng' )
	) );
}
...
add_theme_support( 'custom-header', array(
	'default-image'          => get_bloginfo('template_url').'/images/slogan.png',
...
function dmeng_widgets_init() {
	register_sidebar( array(
		'name' => __( '左侧侧边栏', 'dmeng' ),
		'id' => 'sidebar-1',
		'description' => __( '侧边栏显示在左侧', 'dmeng' ),
		'before_widget' => '<aside id="%1$s" class="'.apply_filters('dmeng_widget_panel_class', 'panel panel-leo widget clearfix %2$s').'">',
		'after_widget' => '</aside>',
...
//~ 载入theme-functions.php文件
require_once( get_template_directory() . '/inc/theme-functions.php' );
//~ 载入inc/functions.php文件
...
require_once( get_template_directory() . '/inc/leo-widgets.php' );
//~ 载入积分
...
function dmeng_get_avatar( $id , $size='40' , $type='' , $lazy=true ){
	...
	if($type==='qq'){
		...
		$avatar_url = sprintf(__('%s', 'dmeng'), huoxing_get_avatar_url(get_avatar($id , $size)));
...
function dmeng_pager($current, $max){
	...
	$pager = '<div class="dmeng-pager clearfix">';
		...
		$pager .= '<div class="btn-group">';
			...
			if($paged>1) $pager .= '<a href="' . add_query_arg('page',$paged-1) . '">'.__('上一页','dmeng').'</a>';
			if($paged<$pages) $pager .= '<a href="' . add_query_arg('page',$paged+1) . '">'.__('下一页','dmeng').'</a>';
			
		$pager .= '</div>';
	
		if ($pages>2 ){
			$pager .= '<div class="btn-group pull-right"><select class="form-control" onchange="document.location.href=this.options[this.selectedIndex].value;">';
				for( $i=1; $i<=$pages; $i++ ){
	...
	if( intval($general_setting['float_button']) ){
		$btn_array = array(
			...
			array(
				'id' => 'editPost',
				'title' => __('趣写作','dmeng'),
                'href' => ( is_user_logged_in() ? add_query_arg(array('tab'=>'post','action'=>'new'), get_author_posts_url(wp_get_current_user()->ID)) : wp_login_url( dmeng_get_current_page_url() ) ).'" target="_blank',				
		...
		if ( !is_single() ) {
			$btn_array[] = array(
				...
				'title'=>__('趣关注','dmeng'),
				'href'=>'javascript:;',
				'html' => '<span class="glyphicon glyphicon-qrcode"></span><div class="qrcode"><img width="150" height="150" src="'.get_template_directory_uri().'/images/qrcode.jpg"><p>欢迎关注'.get_bloginfo('name').'公众号</p></div>'
			);
		}
		if( intval($general_setting['qrcode']) && is_single() ){
			$btn_array[] = array(
				'id' => 'pageQrcode',
				'title' => __('趣关注','dmeng'),
				'href'=>'javascript:;',
				'title'=>__('趣关注','dmeng'),
				'html' => '<span class="glyphicon glyphicon-qrcode"></span><div class="qrcode"><img width="150" height="150" src="'.apply_filters('dmeng_qrcode', 'http://open.dmeng.net/qrcode.png?text='.urlencode(esc_url(dmeng_get_current_page_url())) ).'"><p>扫码分享此文章</p></div>'
			);
		}
       /*	if ( is_single() ) {
			$btn_array[] = array(
		...
		foreach( $btn_array as $btn ){
			...
			$btn_output .= '<a class="btn btn-default" '.$btn_attr.'>'.$btn['html'].'</a>';
		}
		$btn_output .= '</div>';
...
//~ 首页推荐作者列表
function hx_recommend_authors(){
	
	$recommend_author_list = trim(get_option('hx_recommend_author_list'), '|');

	if( !empty($recommend_author_list) ){

		$recommend_author_list = explode('|', $recommend_author_list);

			
	    return $recommend_author_list;
