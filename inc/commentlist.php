...
if ( in_array($comment->comment_type, array('pingback', 'trackback') ) ) {
	...
	echo $comment_time . __(' 链接通知 : ','dmeng') . $author_name;

}else{



if ( $args['echo']!=0 && get_option('show_avatars') ) { ?>
...
<?php } ?>
	<div class="comment-body">
		...
		<div class="comment-meta">
			...
			//~ 回复链接
			comment_reply_link( array_merge( $args, array( 'reply_text' => '<span class="glyphicon glyphicon-share-alt"></span>' . __( '回复','dmeng' ),  'login_text' => '<span class="glyphicon glyphicon-share-alt"></span>' . __('回复','dmeng'), 'add_below' => $add_below, 'depth' => $depth, 'max_depth' => 999999999 ) ) );
			...
			if($user_exists && get_current_user_id()!=$comment->user_id){
				echo '<a class="pm" href="'.add_query_arg('tab', 'message', $author_url).'" title="'.__('私信','dmeng').'" target="_blank"><span class="glyphicon glyphicon-send"></span>'.__('私信','dmeng').'</a>';
			}
