	...
	if( isset($_POST['action']) && sanitize_text_field($_POST['action'])=='test' && wp_verify_nonce( trim($_POST['_wpnonce']), 'check-nonce' ) ) {
		...
		if($open && $host && $user && $pass){
			...
			if(is_email($_POST['dmeng_test_email'])){
				...
				$result = wp_mail( $_POST['dmeng_test_email'],  __('主题SMTP测试邮件','dmeng'),  __('恭喜，SMTP发信成功！','dmeng') );
			...
			dmeng_settings_error('error', __('嗯，这个只是测试主题SMTP发信的，但是你没有开启或者说没有配置好，所以没有发送邮件，请先检查你的SMTP设置！','dmeng'));
