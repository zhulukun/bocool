...
function dmeng_options_home_page(){
  ...
  if( isset($_POST['action']) && sanitize_text_field($_POST['action'])=='update' && wp_verify_nonce( trim($_POST['_wpnonce']), 'check-nonce' ) ) :
    ...
    update_option( 'hx_recommend_author_list', sanitize_text_field($_POST['recommend_author_list']) );
	...
	<form method="post">
		...
		$option->table( array(
			...
			array(
				'type' => 'input',
				'th' => __('推荐作者列表','dmeng'),
				'before' => '<p>'.__('填写作者ID，作者ID之间请使用 | 分隔开，而且 | 前后都不要留空格，最多支持填写10位作者ID。','dmeng').'</p>',
				'key' => 'recommend_author_list',
				'value' => get_option( 'hx_recommend_author_list')
			),
			array(
				'type' => 'checkbox',
				'th' => __('分类列表','dmeng'),
