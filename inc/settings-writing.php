...
function dmeng_options_writing_page(){
  ...
  if( isset($_POST['action']) && sanitize_text_field($_POST['action'])=='update' && wp_verify_nonce( trim($_POST['_wpnonce']), 'check-nonce' ) ) :
	...
	$int_update = array('copyright_status_default', 'post_index', 'post_min_strlen', 'post_max_strlen');
	...
	<form method="post">
		...
		$option->table( array(
			...
			array(
				'type' => 'input',
				'th' => __('投稿的最少字数','dmeng'),
				'before' => '<p>'.__('限制最少字数，一篇文章少于这个字数不允许投稿。一条微博是140，所以默认是140字。','dmeng').'</p>',
				'key' => 'post_min_strlen',
			...
			array(
				'type' => 'input',
				'th' => __('投稿的最多字数','dmeng'),
				'before' => '<p>'.__('限制最多字数，一篇文章超过这个字数不允许投稿。刊物文章一般是2200-12000，所以默认12000字。','dmeng').'</p>',
				'key' => 'post_max_strlen',
