...
function dmeng_login_form(){
	...
	<style>
		...
		.dmeng-open li a{display:block;line-height: 32px;border-radius: 4px;text-indent: 39px;margin: 0 0 10px 0;padding: 5px;background-color: #f5f5f5;background-repeat: no-repeat;background-position:7px 5px;}
...
<ul class="dmeng-open">
	<?php
		foreach( $open_data as $open ){
			echo '<li><a class="'.$open['slug'].'" href="'.dmeng_get_open_login_url($open['slug'], 'login', dmeng_get_redirect_uri()).'">'.sprintf( __('使用%s账号登录', 'dmeng'), $open['title']).'</a> </li>';
		}
	?>
</ul>
	<?php
...
<table class="form-table">
	...
	<?php if($qq){ ?>
	<tr>
		<th scope="row"><?php _e('QQ登录','dmeng');?></th>
		<td>
	<?php  if(dmeng_is_open_qq($user->ID)) { ?>
		<p><?php _e('已绑定','dmeng');?> <a href="<?php echo dmeng_get_open_login_url('qq', 'logout'); ?>"><?php _e('点击解绑','dmeng');?></a></p>
		<?php echo dmeng_get_avatar( $user->ID , '100' , 'qq', false ); ?>
	<?php }else{ ?>
		<a class="button button-primary" href="<?php echo dmeng_get_open_login_url('qq', 'login'); ?>"><?php _e('绑定QQ账号','dmeng');?></a>
	<?php } ?>
		</td>
	</tr>
	<?php } ?>
	...
	<?php if($weibo){ ?>
	<tr>
		<th scope="row"><?php _e('微博登录','dmeng');?></th>
		<td>
	<?php if(dmeng_is_open_weibo($user->ID)) { ?>
		<p><?php _e('已绑定','dmeng');?> <a href="<?php echo dmeng_get_open_login_url('weibo', 'logout'); ?>"><?php _e('点击解绑','dmeng');?></a></p>
		<?php echo dmeng_get_avatar( $user->ID , '100' , 'weibo', false ); ?>
	<?php }else{ ?>
		<a class="button button-primary" href="<?php echo dmeng_get_open_login_url('weibo', 'login'); ?>"><?php _e('绑定微博账号','dmeng');?></a>
	<?php } ?>
		</td>
	</tr>
	<?php } ?>
...
function dmeng_open_template_redirect(){
		...
		$user_ID = get_current_user_id();
			...
			if(is_user_logged_in()){
				...
				if( is_wp_error($insert_user_id) ) {
					...
					wp_die(__('登录失败！ ', 'dmeng'). $redirect_text . '<br> [ '. $insert_user_id->get_error_message() .' ] ' , $die_title , array( 'response'=>503 ) );
			...
			$name = ' <img src="'.get_bloginfo('template_url').'/images/weibo_32x32.png" > 微博 ';	
				...
				if( isset($_GET['wpnonce']) && wp_verify_nonce( trim($_GET['wpnonce']), $type.'_logout' ) ){
					...
					wp_die(
						...
						__('解除账号绑定','dmeng') , array( 'response'=>503 )
					);
	...
	function dmeng_connect_check($str=''){
		if(empty($str)){
			wp_die(__('服务器无法连接开放平台，请重试或联系管理员！', 'dmeng').dmeng_get_redirect_text(), __('无法连接开发平台', 'dmeng'), array( 'response'=>503 ));
		}
		return $str;
	}
