...
function dmeng_redirect_wp_admin(){
        if( is_admin() && is_user_logged_in() && !current_user_can( apply_filters('dmeng_admin_base_cap', 'publish_pages') ) && ( !defined('DOING_AJAX') || !DOING_AJAX )  ){
...
function dmeng_edit_post_link($url, $post_id){
        if( !current_user_can( apply_filters('dmeng_admin_base_cap', 'publish_pages') ) ){
...
//~ 添加昵称搜索
function dmeng_user_search_columns($search_columns, $search, $this){
	if(!in_array('display_name', $search_columns)){
		$search_columns[] = 'display_name';
	}
	return $search_columns;
}
add_filter('user_search_columns', 'dmeng_user_search_columns' , 10, 3);
...
function dmeng_user_profile_widget(){
	...
	if( $current_user->user_email != $current_user->dmeng_verify_email ){
		...
		$li_output .= '<li><a href="'.dmeng_get_user_url('profile').'#pass">'.__('【重要】请验证一个邮箱用以接收通知','dmeng').'</a></li>';
	...
	foreach( $shorcut_links as $shorcut ){
		 $shorcut_html .= '<a href="'.$shorcut['url'].'" data-no-instant>'.$shorcut['title'].' &raquo;</a>';
	}
	 $shorcut_html .= '</li>';
