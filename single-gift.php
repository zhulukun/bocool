...
if( !empty($_GET['api']) && $_GET['api']=='log' ){
	...
	$title = sprintf( __('《%s》全部兑换记录', 'dmeng'), esc_html(get_the_title(get_queried_object_id())) );
	...
	if($is_can_edit_post){
		...
		if( $exchange ){
			foreach( $exchange as $exterm ){
				...
				$buyers_list .= sprintf( __(' %1$s 在 %2$s 兑换了一个，唯一识别码是 %3$s ', 'dmeng'), 
			...
			$buyers_list .= __('没有找到兑换记录', 'dmeng');
			
		}
		
	}else{
		
		$buyers_list .= __('当前帐号没有查看权限', 'dmeng');
		
	}
	
	$buyers_list .= '<br><br><a href="'.get_permalink(get_queried_object_id()).'">'.__('&laquo; 返回查看内容', 'dmeng').'</a> / <a href="'.add_query_arg('api', 'check', get_post_type_archive_link( 'gift' ) ).'">'.__('查询兑换信息', 'dmeng').'</a>';
	...
	<div class="row">
			<?php 
				while ( have_posts() ) : the_post();
					...
					$output .= '<div class="entry-thumbnail gift-thumbnail"><a href="'.get_permalink().'" title="'.get_the_title().'"><img src="'.dmeng_script_uri('grey_png').'" data-original="'.$thumbnail.'" alt="'.get_the_title().'"></a></div>';
					...
					if( $uid ){
						...
						if( $exchange ){
							foreach( $exchange as $exterm ){
									...
									$buyers_list .= '<p>'.sprintf( __(' %1$s 在 %2$s 兑换了一个，唯一识别码是 %3$s ', 'dmeng'), 
																			( $exchang_id && $exterm->user_id==$uid ) ? __('你', 'dmeng') : get_the_author_meta( 'display_name', $exterm->user_id),
																			$exterm->meta_value , 
							...
							if( $exchang_id==0 ) $buyers_list .= '<p><a href="'.add_query_arg('api', 'log', get_permalink()).'" target="_blank">'.__('查看全部记录', 'dmeng').'</a> / <a href="'.add_query_arg('api', 'check', get_post_type_archive_link( 'gift' ) ).'" target="_blank">'.__('查询兑换信息', 'dmeng').'</a></p>';
						}
						
						if($buyers_list) $output .= '<div class="page-header">'.__('兑换记录', 'dmeng').'</div><div>'.$buyers_list.'</div>';
					}
	           ...
	           if ( function_exists( 'show_paginate' ) ) { show_paginate(); } 
